...
####  App as playground
The app should be implemented as a playground that the AI agent can run experiments in and then inspect and reason about the experiments. The chat is essentially a classic loop of: observe → decide → act → observe → decide → act → ... . Which corresponds to the gymnasium loop
