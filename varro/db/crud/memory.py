...
class CrudMemory:
    """
    Focused CRUD for Memory using SQLModel/SQLAlchemy.
    ...
    def _visible(self):
        return Memory.user_id.in_([self.user_id, SYSTEM_USER_ID])

    def get_by_path(self, path: str) -> Optional[Memory]:
        with Session(self.engine) as session:
            return session.exec(
                select(Memory).where(Memory.path == path, self._visible())
            ).one_or_none()

    def list_paths_under_prefix(self, prefix: str) -> list[str]:
        with Session(self.engine) as session:
            like_prefix = prefix.rstrip("/") + "/%"
            q = select(Memory.path).where(
                (Memory.path == prefix) | (Memory.path.like(like_prefix)),
                self._visible(),
            )
        ...
        with Session(self.engine) as session, session.begin():
            stmt = (
                ...
                .on_conflict_do_update(
                    index_elements=[Memory.path, Memory.user_id],
    ...
    # ---- Update text ----
    def set_text(self, path: str, new_text: str) -> Optional[Memory]:
        with Session(self.engine) as session, session.begin():
            stmt = (
                update(Memory)
                .where(Memory.path == path, Memory.user_id == self.user_id)
    ...
    def delete_file(self, path: str) -> int:
        with Session(self.engine) as session, session.begin():
            obj = session.exec(
                select(Memory).where(
                    (Memory.path == path), (Memory.user_id == self.user_id)
                )
            ).one_or_none()
            if obj is None:
                return 0
    ...
    def rename_file(self, old_path: str, new_path: str) -> bool:
        with Session(self.engine) as session, session.begin():
            ...
            if session.exec(
                select(Memory).where(Memory.path == new_path)
            ).one_or_none():
                raise ValueError(f"Target already exists: {new_path}")

            stmt = (
                update(Memory)
                .where(Memory.path == old_path, Memory.user_id == self.user_id)
    ...
    def delete_all_system_files(self) -> int:
        with Session(self.engine) as session, session.begin():
            stmt = delete(Memory).where(Memory.user_id == SYSTEM_USER_ID)
            return session.exec(stmt).rowcount


memory = CrudMemory(engine, SYSTEM_USER_ID)
