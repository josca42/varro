...
- `Jupyter` cell execution now runs in silent mode so implicit last-expression repr output is suppressed from tool `return_value`; text output is print/stdout only. Figures/dataframes are surfaced via `show=[...]`.
- Geo boundary data now has shared process cache in `varro.agent.utils.get_geo(layer)`. Jupyter initial imports include `get_geo`, and each call returns `gdf.copy(deep=False)` so cells get a cheap wrapper without mutating the cached base object.
...
- When `data/dst/dimension_links/{TABLE}.json` is missing or empty, `ColumnValues(..., for_table=...)` now falls back to simple column inference (name similarity, then kode overlap) using `context/column_values/{table}/*.parquet`.
- Agent system prompt now injects a dynamic `<available_skills>` section from per-user workspace files in `/skills/**/SKILL.md`, extracting only front-matter `name` and `description` plus file location metadata.
